...
static int __devinit cci_probe(struct platform_device *pdev)
{
	...
	//cci->base =  sunxi_map_kernel((unsigned int)res->start, (unsigned int)resource_size(res));
	cci->base = IO_ADDRESS((unsigned int)res->start);
    if (!cci->base) {
		ret = -EIO;
		goto eremap;
	}
...
static int __devexit cci_remove(struct platform_device *pdev)
{
	...
	if(cci->base)
	sunxi_unmap_kernel(cci->base,(unsigned int)res->start, (unsigned int)resource_size(res));
	kfree(cci);
	return 0;
}
