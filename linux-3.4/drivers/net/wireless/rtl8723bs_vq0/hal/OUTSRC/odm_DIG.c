...
)
{
	...
	if(pDM_Odm->SupportICType & ODM_IC_11N_SERIES)
		...
		ODM_SetBBReg(pDM_Odm, rFPGA0_XB_LSSIReadBack, bMaskLWord, (u1Byte)(H2L<<8) | (u1Byte)L2H);
}
...
)
{
	...
	ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("EDCCA State = %d \n", State));
...
)
{
	...
	{
		if(pDM_Odm->adaptivity_flag == TRUE && pDM_Odm->NHM_cnt_0 <= 100)
		{
			odm_MACEDCCAState(pDM_Odm, ODM_DONT_IGNORE_EDCCA);
			pDM_Odm->tolerance_cnt = 0;
#if(DM_ODM_SUPPORT_TYPE & (ODM_AP|ODM_ADSL))
		...
		else if((pDM_Odm->NHM_cnt_0 >= 50) && (pDM_Odm->VXD_bLinked) && ((pDM_Odm->NHMCurTxOkcnt>>10) > 1))		// clean environment and VXD linked and Tx TP>1Mb
		{
			pDM_Odm->adaptivity_flag = TRUE;
			odm_MACEDCCAState(pDM_Odm, ODM_DONT_IGNORE_EDCCA);
			pDM_Odm->tolerance_cnt = 0;
...
)
{
	...
	{
		pDM_Odm->TH_L2H_ini = 0xf5; // -11   : detection threshold = -71dBm
	}
	else
...
)
{
	...
	BOOLEAN EDCCA_State = FALSE, isInterference;
	...
	isInterference = odm_CheckEDCCABit(pDM_Odm);
	
	if(((!pDM_Odm->bLinked) && (!isInterference)) || (*pDM_Odm->pChannel >= 149))		// Band4 -> for AP : mode2, for sd4 and sd7 : turnoff adaptivity
	{
	...
	if(pDM_Odm->bLinked && pDM_Odm->Carrier_Sense_enable == FALSE && pDM_Odm->NHM_disable == FALSE)
		odm_NHMBB(pDM_Odm);

	ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("BandWidth=%s, IGI_target=0x%x, EDCCA_State=%d, isInterference = %d\n",
		(*pDM_Odm->pBandWidth==ODM_BW80M)?"80M":((*pDM_Odm->pBandWidth==ODM_BW40M)?"40M":"20M"), IGI_target, EDCCA_State, isInterference));
