...
echo "event num $event_num"

mkdir -p ${LICHEE_BR_DIR}/target/${LICHEE_PLATFORM}/rootfs/sensors_cache
echo  "${LICHEE_BR_DIR}/target/${LICHEE_PLATFORM}/rootfs/etc/profile"
sed   -i 's/export TSLIB_TSDEVICE=\/dev\/input\/event.*/export TSLIB_TSDEVICE=\/dev\/input\/event'$event_num'/g' ${LICHEE_BR_DIR}/target/${LICHEE_PLATFORM}/rootfs/etc/profile
