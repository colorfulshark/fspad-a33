...
static int init_layer(void)
{
    ...
    memset(&fb_para, 0, sizeof(disp_fb_create_info));
...
int main(int argc, char *argv[])
{
    ...
    args[0] = 0;
    output_type = ioctl(disp, DISP_CMD_GET_OUTPUT_TYPE,(void*)args);
        ...
        if (ret == 1) {
            ...
            if (output_type == DISP_OUTPUT_TYPE_LCD) {
               ...
               ret = init_layer();
                ...
                /* set hdmi on hdmi is start work */ 
                args[0] = 1;
                ret = ioctl(disp, DISP_CMD_HDMI_ENABLE, args);
                if (ret < 0) {
                ...
                if (ret != 0) {
                    ...
                    exit_layer();
                    args[0] = 1;
                    ioctl(disp, DISP_CMD_HDMI_DISABLE, args);
                    goto err;
                }
                status = 1;
                SEND_CMD_PIPE_OK();
