...
int update_boot_para(void* dtb_base)
{
	int err;
	...
	err = fdt_fixup_memory(dtb_base, CONFIG_SYS_SDRAM_BASE ,gd->ram_size);
	if(err < 0)
	{
		return -1;
	}
...
int do_boota_linux (struct andr_img_hdr *hdr,void * dtb_base)
{
	...
	if(0 != update_boot_para(dtb_base))
	{
		printf("boota: update boot para fail\n");
		return -1;
	}
...
int do_boota (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	struct  andr_img_hdr *fb_hdr = (struct andr_img_hdr *)addr;
	image_header_t *hdr =(image_header_t *)(addr + CFG_ANDROID_IMAGE_PAGE_SIZE);
	...
	debug("ramdisk size: 0x%x \n", fb_hdr->ramdisk_size);
	debug("ramdisk addr: 0x%x \n", fb_hdr->ramdisk_addr);

	debug("second size: 0x%x \n", fb_hdr->second_size);
	...
	debug("moving kernel from %lx to: %x, size 0x%x\n", kaddr, fb_hdr->kernel_addr, fb_hdr->kernel_size);
	debug("moving ramdisk from %lx to: %x, size 0x%x\n", raddr, fb_hdr->ramdisk_addr, fb_hdr->ramdisk_size);
	memcpy2((void*) (long)fb_hdr->kernel_addr, (const void *)kaddr, fb_hdr->kernel_size);
	memcpy2((void*) (long)fb_hdr->ramdisk_addr, (const void *)raddr, fb_hdr->ramdisk_size);
