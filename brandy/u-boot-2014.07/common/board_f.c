...
{
	ulong head_align_size = get_spare_head_size();
	ulong text_section_begin = (CONFIG_SYS_TEXT_BASE + head_align_size);
	
	//copy head data to reloc address
	memcpy((void *)gd->relocaddr, (void *)_TEXT_BASE, sizeof(struct spare_boot_head_t));

	//set code relocaddr to start of the code section  
	gd->relocaddr = gd->relocaddr + head_align_size;
	gd->reloc_off = gd->relocaddr - text_section_begin;
