					...
					u8 *src_addr, u32 src_len)
{
	...
	p_iv 	= (u8 *)(((ulong)iv_buff + 31)&(~31));
	p_sign =  (u8 *)(((ulong)sign_buff + 31)&(~31));
	//set mode
	...
	//set src addr
	writel((u32)(ulong)src_addr	, SS_DATA_SRC_LOW_ADR);
	writel(0		        , SS_DATA_SRC_HIGH_ADR);
	//set dest addr
	writel((u32)(ulong)p_sign, SS_DATA_DST_LOW_ADR);
	writel(0	      , SS_DATA_DST_HIGH_ADR);
	//set src len
	//while((*(volatile int *)0)!=1);
	total_len = __sha256_padding(src_len,(u8 *)src_addr);	//计算明文长度

	flush_cache((u32)(ulong)src_addr, total_len);
	flush_cache((u32)(ulong)sign_buff, sizeof(sign_buff));
	flush_cache((u32)(ulong)iv_buff, sizeof(iv_buff));

	writel(total_len/4,SS_DATA_LEN);
	//set IV
	writel((u32)(ulong)p_iv, SS_PM_LOW_ADR);
